...
function (IBEX_INIT_COMMON)
  ...
  if (MSVC)
    ...
    check_cxx_compiler_flag ("-fno-fast-math" COMPILER_SUPPORTS_FNO_FAST_MATH)
    ...
    check_cxx_compiler_flag ("-ffp-contract=off" COMPILER_SUPPORTS_FFP_CONTRACT_OFF)  
    if (COMPILER_SUPPORTS_FFP_CONTRACT_OFF)
      set (CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -ffp-contract=off")
      set (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -ffp-contract=off")
    endif ()
    check_cxx_compiler_flag ("-ffp-mode=full" COMPILER_SUPPORTS_FFP_MODE_FULL)  
    if (COMPILER_SUPPORTS_FFP_MODE_FULL)
      set (CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -ffp-mode=full")
      set (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -ffp-mode=full")
    endif ()
    check_cxx_compiler_flag ("-fp-model=strict" COMPILER_SUPPORTS_FP_MODEL_STRICT)  
    if (COMPILER_SUPPORTS_FP_MODEL_STRICT)
      set (CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -fp-model=strict")
      set (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -fp-model=strict")
    endif ()
    check_cxx_compiler_flag ("-fp:strict" COMPILER_SUPPORTS_FP_STRICT)  
    if (COMPILER_SUPPORTS_FP_STRICT)
      set (CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -fp:strict")
      set (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -fp:strict")
    endif ()
    check_cxx_compiler_flag ("-mpc64" COMPILER_SUPPORTS_MPC64)  
    if (COMPILER_SUPPORTS_MPC64)
      set (CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -mpc64")
      set (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -mpc64")
    endif ()
    ...
    if (APPLE)
      ...
      if (COMPILER_SUPPORTS_WNO_UNDEFINED_VAR_TEMPLATE)
        set (CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -Wno-undefined-var-template")
        set (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -Wno-undefined-var-template")
      endif ()
    endif ()
    # Due to warnings when cross-compiling on macOS with filib and GAOL
    if (APPLE)
      #check_cxx_compiler_flag ("-Wno-ignored-pragmas" COMPILER_SUPPORTS_WNO_IGNORED_PRAGMAS)  
      #if (COMPILER_SUPPORTS_WNO_IGNORED_PRAGMAS)
      #  set (CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -Wno-ignored-pragmas")
      #  set (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -Wno-ignored-pragmas")
      #endif ()
    endif ()
    set (CMAKE_C_FLAGS_DEBUG "-Wall ${CMAKE_C_FLAGS_DEBUG} -DDEBUG")
    set (CMAKE_CXX_FLAGS_DEBUG "-Wall ${CMAKE_CXX_FLAGS_DEBUG} -DDEBUG")
  endif ()
  # Claim IEEE 754 double compatibility, for filib
  set (CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -D__STDC_IEC_559__=1")
  set (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -D__STDC_IEC_559__=1")

  #
  # This scope vs parent scope...
  #
  ...
  set (CMAKE_C_FLAGS_MINSIZEREL "${CMAKE_C_FLAGS_MINSIZEREL}" PARENT_SCOPE)
